...
contract DividendModule is CheckpointModule {
	...
	function issueDividend(uint256 _claimPeriod) external onlyIssuer payable {
		require(dividendTime < now);
		require(claimExpiration == 0);
		require(msg.value > 0);
		claimExpiration = now.add(_claimPeriod);
		dividendAmount = msg.value;
		totalSupply = totalSupply.sub(_getBalance(address(token.issuer())));
		emit DividendIssued(dividendTime, msg.value);
	}

	function claimDividend(address payable _beneficiary) public {
	...
	function claimMany(address payable[] calldata _beneficiaries) external {
